...
def build_dataset(
  ...
  tmp = pd.concat([
      ...
      cite_edges.loc[:, "target"],
      ref_edges.loc[:, "target"],
  ])
  ...
  false_source = node_authors.sample(
      frac=test_true_refs.shape[0] / node_authors.shape[0],
      random_state=random_state,
      replace=True,
      axis=0,
  ).reset_index()
  false_target = node_papers.sample(
      frac=test_true_refs.shape[0] / node_papers.shape[0],
      random_state=random_state,
      replace=True,
      axis=0,
  ).reset_index()
  ...
  test_refs = pd.concat([
      test_true_refs,
      test_false_refs.iloc[:min(len(false_source), len(false_target))],
  ])
...
def build_graph(device):
  ...
  train_refs, test_refs, cite_edges, coauthor_edges = build_dataset(
      citation, existing_refs, coauthor)
  ...
  graph_data = {
      ...
      rel_list[1]: (
          torch.cat([cite_tensor[:, 0], cite_tensor[:, 1]]),
          torch.cat([cite_tensor[:, 1], cite_tensor[:, 0]]),
      ),
      rel_list[2]: (
          torch.cat([coauthor_tensor[:, 0], coauthor_tensor[:, 1]]),
          torch.cat([coauthor_tensor[:, 1], coauthor_tensor[:, 0]]),
      ),
